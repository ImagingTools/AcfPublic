	...
	m_application(application)
{
    ...
    applicationArguments = m_parent.GetApplicationArguments(serviceArgc, serviceArgv);

    for (QString argument : applicationArguments)
    {
        m_applicationArguments.push_back(argument.toUtf8());
    }
...
QVector<char*> CServiceApplicationComp::CService::GetApplicationArguments() const
{
    ...
    for (int argIndex = 0; argIndex < m_applicationArguments.count(); argIndex++){
        argv[argIndex] = (char*)m_applicationArguments[argIndex].data();
    }
