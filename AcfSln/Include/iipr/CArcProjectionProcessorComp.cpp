			...
			imeas::IDataSequence& results)
{
	...
	int arcLength = M_PI * bitmapArc.GetRadius() * (bitmapArc.GetAngleWidth() / 180);
	...
	double angleDiff = bitmapArc.GetAngleWidth() / arcLength;

	double angleStart = bitmapArc.GetStartAngle();
	double angleWidth = bitmapArc.GetAngleWidth();
