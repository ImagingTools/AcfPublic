...
{
	// create gui data models and listen to their signals to support undo management
	for (int i = 0; i < 3; i++){
...
void CXpcEditorComp::PickFileNameForSelection(QListView* listView, bool SelectDirectories, iser::IFileTypeInfo* typeInfo)
{
	if (!listView){
		return;
	}
	...
	QFileInfo fileInfo(documentDir.absoluteFilePath(unrolledPath));
	...
	if (result == QDialog::Accepted){
		...
		if (filenames.size()){
			listView->model()->setData(index, documentDir.relativeFilePath(filenames[0]));
		}
		DoUpdateModel();
	}
}
