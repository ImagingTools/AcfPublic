	...
	if (selectionPtr != NULL){
		...
		if ((constraintsPtr != NULL) && (selectedIndex >= 0) && (selectedIndex < constraintsPtr->GetOptionsCount())){
			...
			if (m_labelWidthAttrPtr.IsValid()){
				...
				optionDescription = fontMetrics.elidedText(optionDescription, Qt::ElideRight,
					*m_labelWidthAttrPtr > GetQtWidget()->minimumWidth() ? *m_labelWidthAttrPtr : GetQtWidget()->minimumWidth());
			}
